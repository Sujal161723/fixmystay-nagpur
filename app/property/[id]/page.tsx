...
export default function PropertyDetail() {
  ...
  const trackLead = async (actionType: string) => {
    ...
    try {
      await addDoc(collection(db, "leads"), {
        ...
        action: actionType, // "WhatsApp" or "Call"
        timestamp: serverTimestamp(),
        area: property.area || "Unknown"
      });
      console.log(`${actionType} lead recorded!`);
    } catch (error) {
  ...
  const waMessage = encodeURIComponent(`Hi, I'm interested in "${property.title}" on FixMyStay.`);
      ...
      <div className="max-w-6xl mx-auto mt-6 px-4">
        <div className="bg-white p-6 rounded-t-3xl border-b border-slate-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <p className="flex items-center gap-1 text-slate-500 font-semibold mt-1 italic text-sm"><MapPin size={14}/> {property.area}, Nagpur</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-1 bg-white border-x border-b border-slate-200 rounded-b-3xl overflow-hidden shadow-sm">
          ...
          <div className="lg:col-span-5 p-8 grid grid-cols-2 gap-y-8 gap-x-4 bg-white self-center">
            <QuickFact icon={<Home size={20}/>} label="Configuration" value={property.category} />
            <QuickFact icon={<Maximize size={20}/>} label="Landmark" value={property.landmark} />
            <QuickFact icon={<Layers size={20}/>} label="Furnishing" value="Semi-Furnished" />
            <QuickFact icon={<Calendar size={20}/>} label="Available" value="Immediately" />
          </div>
        </div>
        ...
        <div className="mt-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-24 bg-white p-8 rounded-3xl shadow-xl border-2 border-blue-50 space-y-6">
              <div className="space-y-4">
                {/* --- CALL BUTTON WITH TRACKING --- */}
                <a 
                  href={`tel:${property.phone}`} 
                  onClick={() => trackLead("Call")}
                  className="w-full bg-blue-600 text-white py-5 rounded-2xl font-black text-sm flex items-center justify-center gap-2 hover:bg-blue-700 transition-all active:scale-95"
                >
                ...
                <a 
                  ...
                  className="w-full border-2 border-green-500 text-green-600 py-5 rounded-2xl font-black text-sm flex items-center justify-center gap-2 hover:bg-green-50 transition-all active:scale-95"
                >
